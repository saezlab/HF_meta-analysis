...
GSEA_filtering = GSEA %>% 
  arrange(padj_global) %>%
  slice(1:50) %>%
  arrange(-log10(padj_global) * sign(NES)) %>%
  mutate(pathway = strtrim(pathway,40)) %>%
  mutate(dir_reg = sign(ES),
         log_pval = -log10(padj_global),
         gset = pathway)
...
progeny_filtering = progeny %>%
  arrange(-log10(adj_pvalues + 0.00000001) * sign(progeny_scores)) %>%
  mutate(dir_reg = sign(progeny_scores),
         log_pval = -log10(adj_pvalues + 0.00000001) + 0.05,
         gset = pathway) %>%
